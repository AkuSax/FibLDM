...
def main():
    ...
    # --- Load Models ---
    tokenizer = CLIPTokenizer.from_pretrained(os.path.join(args.pretrained_model_path, "tokenizer"))
    text_encoder = CLIPTextModel.from_pretrained(os.path.join(args.pretrained_model_path, "text_encoder"))
    vae = AutoencoderKL.from_pretrained(args.pretrained_vae_path)
    unet = UNet2DConditionModel.from_pretrained(os.path.join(args.pretrained_model_path, "unet"))
